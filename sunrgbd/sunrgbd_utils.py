...
def load_depth_points(depth_filename):
    if os.path.splitext(depth_filename)[-1] == '.npy':
        depth = np.load(depth_filename)
    elif os.path.splitext(depth_filename)[-1] == '.mat':
        depth = sio.loadmat(depth_filename, verify_compressed_data_integrity=False)['x'].astype(np.float32)
    elif os.path.splitext(depth_filename)[-1] == '.txt':
        depth = np.loadtxt(depth_filename)
    else:
        assert False, "unknown file format %s " % (depth_filename)
    return depth
...
def compute_box_3d_obj_array(obj_array):
    '''
        cx, cy, cz, l, w, h, heading_angle
        Returns:
            corners_3d: (8,3) array in in upright depth coord.
    '''

    cx, cy, cz, l, w, h, heading_angle = obj_array
